...
public class RegistrarseActivity extends AppCompatActivity {
    ...
    public void registar(View view) {
        ...
        if(!fname.isEmpty()&&!lname.isEmpty()&&!mail.isEmpty()&&!pass1.isEmpty()&&!pass2.isEmpty()){
            if(buscar()){
                Toast.makeText(this, "Un usuario con este nombre ya existe",Toast.LENGTH_SHORT).show();
            }else{
    ...
    public boolean buscar(){
        ...
        Cursor fila = BaseDeDatos.rawQuery("select nombre from usuarios where nombre ="+nombre,null);
        if(fila.moveToFirst()){
            encontrado = true;
            BaseDeDatos.close();
        }else{
